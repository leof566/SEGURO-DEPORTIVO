...
function to8Digits(dni) {
  const only = (dni||"").replace(/\D/g, "");
  return only.slice(-8);
}
...
function parseCSV(text){
  ...
  for (let i=1;i<lines.length;i++){
    ...
    out.push({
      dni: to8Digits(cols[idxDni]||""),
      apellido: cols[idxApe]||"",
      nombre: cols[idxNom]||"",
      estado: (cols[idxEst]||"PAGO").toUpperCase()==="DEBE" ? "DEBE":"PAGO"
    });
  }
  return out;
}
...
function openModal(editIndex = null) {
  ...
  if (editIndex === null) {
    els.modalTitle.textContent = "Agregar Persona";
    els.modalForm.reset();
  } else {
    els.modalTitle.textContent = "Editar Persona";
    const p = state.data[editIndex];
    els.dniInput.value = p.dni;
    els.apellidoInput.value = p.apellido;
    els.nombreInput.value = p.nombre;
    els.estadoInput.value = p.estado;
  }
  els.modal.showModal();
}
...
function upsertFromModal(e) {
  e.preventDefault();
  const dni = to8Digits(els.dniInput.value);
  const apellido = els.apellidoInput.value.trim();
  const nombre = els.nombreInput.value.trim();
  const estado = els.estadoInput.value;

  if (!dni || dni.length < 7 || !apellido || !nombre) {
    alert("Por favor completa DNI (7 u 8 dígitos), Apellido y Nombre.");
    return;
  }
  ...
  if (state.editIndex === null) {
    ...
    if (i >= 0) {
      if (!confirm("Ya existe un registro con ese DNI. ¿Deseas reemplazarlo?")) return;
      state.data[i] = person;
    } else {
      state.data.push(person);
    }
  } else {
    state.data[state.editIndex] = person;
  }
  save();
  els.modal.close();
  filter();
...
els.tbody.addEventListener("click", (e) => {
  ...
  if (action === "toggle") {
    state.data[idx].estado = state.data[idx].estado === "PAGO" ? "DEBE" : "PAGO";
    save(); filter();
  }
  if (action === "delete") {
    if (confirm("¿Quitar esta persona?")) {
...
els.exportJsonBtn.addEventListener("click", () => {
  ...
  a.href = url;
  a.download = "asegurados-backup.json";
  a.click();
...
els.importJsonInput.addEventListener("change", async (e) => {
  ...
  try {
    ...
    for (const it of arr) {
      ...
      it.dni = to8Digits(it.dni);
      it.estado = it.estado === "DEBE" ? "DEBE" : "PAGO";
    }
    state.data = arr;
    save();
    filter();
...
function countDebtors() {
  return state.data.filter(p => p.estado === "DEBE").length;
}
els.alertDebtorsBtn.addEventListener("click", async () => {
  const n = countDebtors();
  alert(`Deudores: ${n}`);
  if ("Notification" in window) {
    try {
      ...
      if (perm === "granted") {
        new Notification("Asegurados", { body: `Tenés ${n} deudor(es).`, icon: "icons/icon-192.png" });
      }
    } catch(e) {}
  }
});
